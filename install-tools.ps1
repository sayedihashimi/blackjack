...
function DeleteCacheFolders{
    [cmdletbinding()]
    param(
        [string[]]$toolNames = ("SayedHa.Blackjack.Cli","SayedHa.Roulette.Cli"),
        [string[]]$commandName = ("blackjack","roulette")
    )
    process{
...
try {
    ...
    Write-Host "An error occurred deleting cache folders: "
    Write-Host $_
}
 
# TODO: Both commands not working on linux, fix later
[string[]]$projects = (Join-Path $scriptDir 'src\SayedHa.Blackjack.Cli\SayedHa.Blackjack.Cli.csproj'),(Join-Path $scriptDir 'src\SayedHa.Roulette.Cli\SayedHa.Roulette.Cli.csproj')
foreach($p in $projects){
    'Building and installing tool for project at: "{0}"' -f $p | Write-Output
    dotnet build $p -t:Pack -t:InstallTool -p:Configuration=Release
}