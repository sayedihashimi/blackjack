...
namespace SayedHa.Blackjack.Cli {
    public class SpectreConsolePlayer : Player {
        ...
        public override HandAction GetNextAction(Hand hand, DealerHand dealerHand) => AnsiConsole.Prompt(
            ...
            .Title($@"Your hand:{hand.ToString(hideFirstCard: false, useSymbols: true, includeScore: IncludeScoreInOutput, includeBrackets: false, includeResult: false).EscapeMarkup()} Dealer hand: {hand.ToString(hideFirstCard: true, useSymbols: true, includeScore: false, includeBrackets: false, includeResult: false).EscapeMarkup()}
